# Metacells Sharpening Vignette

Sharpening takes metacells computed by something else — an MC2, `scvi` or SEACells partition — and improves them, using
the `Metacells.jl` pipeline. This is a Jupyter notebook which runs that pipeline on a gastrulation dataset, in Python,
through `metacellspy`.

*NOTE: THIS IS WIP. DO NOT USE.*

## Reading it

Reading needs nothing installed. Both forms hold the same thing, and each is a single self-contained file which needs no
network access to view:

* [HTML](https://tanaylab.github.io/metacells-sharpening-vignette/sharpening.html)
* [PDF](https://tanaylab.github.io/metacells-sharpening-vignette/sharpening.pdf)

## Installing it

See [INSTALL.md](INSTALL.md). It describes three ways of installing — into the Python and Julia you already use, into a
conda environment which brings its own, or into a Docker image which brings the operating system as well — and which to
choose. Eight packages are involved, three Python and five Julia, and no package index carries a working version of any
of them, which is why installing is a document of its own.

## Downloading the data

TODO

## Running it

With all the above done:

```
git clone https://github.com/tanaylab/metacells-sharpening-vignette
cd metacells-sharpening-vignette
conda activate metacells-sharpening   # only if you installed that way
jupyter lab sharpening.ipynb
```

The first cell imports the three Python packages and prints a value read from Julia, so an environment which is not set
up fails there rather than somewhere in the middle of the pipeline.

The notebook reads `assigned_cells.h5ad` and `type_colors.csv` from `input/`, which are not in this repository, the
first being far too large for one. You can equally point it at your own data, in which case you need nothing from this
repository at all — what you installed is the whole pipeline.

## The build

The documents are generated by `make`, which describes itself:

```
make help
```

`make docs` generates the HTML and the PDF from the committed notebook. `make ci` also checks them, and is what the CI
server runs, so running it locally does exactly what happens there.

Executing the notebook is `make run`, and is separate because it runs the whole pipeline. Even then, only a notebook
whose *code* changed is executed: the fingerprint of the code cells is recorded in the notebook itself, so rewording a
paragraph regenerates the documents without recomputing anything. Executing is done in place, so the outputs are
replaced while the prose around them stays as it is.

Only `make run` needs the vignette's own environment. Generating the documents is a separate tool chain, pinned in
`requirements.txt`, and needs neither the eight packages nor the Julia they talk to nor the input data — the notebook is
committed with its outputs, and converting it is all that happens. `EXECUTE_IN` is how the vignette's environment is
entered, and defaults to the conda install:

```
make run                  # the conda environment
make run EXECUTE_IN=      # the Python and Julia you already use
```

## Developing it

A notebook is a poor place to develop in, since every run starts from the top and this pipeline is far too expensive
for that. The code of each cell is therefore a script in `steps`, and `steps/Makefile` lists them in the order they
run — that list is what the notebook's code *is*. Each script is in two parts: above the marker, the context the
notebook would have in memory by then; below it, what actually becomes the cell. Only the lower half is published, so
the context may be as repetitive as it likes.

```
make -C steps list         # the steps, in order
make -C steps              # run whichever are out of date
make -C steps import_cells # run up to that one, and no further
```

A step leaves its results on disk, in `dafs`, since a script cannot leave anything in memory for the next one. That is
what lets any step be re-run on its own.

`make embed` then puts each step's cell into the notebook cell which names it — in the cell's own metadata, so that
adding a paragraph or reordering the prose cannot put one step's code into another's slot. `make check` reports every
way the two can disagree: a cell naming no step, a cell naming a step which does not exist, two cells naming one step,
a step with no cell, and cells in an order the steps do not run in.

There are two environments, and they are separate on purpose. Running a step or executing the notebook needs the
vignette's own, which [INSTALL.md](INSTALL.md) describes. Everything else — the tests, generating the documents, and
checking that the committed ones match the committed code — needs only the document tool chain, which `make ci_env`
builds from the pinned `requirements.txt`:

```
make ci_env  # build the document tool chain, once
make ci      # what the CI server runs: test, generate the documents, check them
```

`make ci_env` does nothing when `requirements.txt` has not changed, and the CI server builds it the same way, so the
gate runs against the same versions in both places — which matters because it compares generated bytes.

Each command of the gate names the interpreter it wants out of `.ci_env`, so nothing depends on your path. `make run`
is the exception and uses the vignette's environment; do not run it from inside another virtual environment, since
`conda run` does not override one which is already first in your path.

Generating a PDF prints the HTML from a headless chromium, which `make ci_env` fetches. On a distribution older than
that browser — RHEL 8 and its relatives — it also needs `LD_LIBRARY_PATH` pointing at a copy of the system libraries it
expects, in the same way, and for the same reason, that the vignette's own install may need one for Julia's C++
runtime.
